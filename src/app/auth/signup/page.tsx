...
import { Cpu, Lock, Mail, User, ArrowRight, AlertCircle, CheckCircle2, ShieldCheck, Sparkles, Check } from 'lucide-react';
...
export default function SignUpPage() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Account created successfully! Redirecting to your laboratory...');
      setTimeout(() => {
        router.push('/?tab=lab');
      }, 1500);
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-[#FFF9F6] text-[#0F151D] flex flex-col justify-center items-center px-4 py-8 sm:py-12 font-sans selection:bg-[#FF7448] selection:text-white">
      ...
      <div className="w-full max-w-4xl bg-white border border-[#F0E6E1] rounded-[36px] sm:rounded-[48px] shadow-2xl shadow-[#FF7448]/5 overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 p-8 sm:p-12 flex flex-col justify-between space-y-8">
          <div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs sm:text-sm text-slate-500">
                Unlock persistent laboratory sessions, progress tracking, and AI tutoring.
              </p>
            </div>
          </div>
          ...
          {successMsg && (
            <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs flex items-center space-x-2.5 animate-in fade-in">
          ...
          <div className="pt-2 text-center text-xs text-slate-500">
            ...
            <Link href="/auth/login" className="text-[#FF7448] hover:underline font-bold">
              Sign In Here
            </Link>
          </div>
        </div>
